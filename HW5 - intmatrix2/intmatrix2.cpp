#include "intmatrix2.h"
	...
	intmatrix2::intmatrix2(const char* string_matrix) {
	...
	//copy constructor
	intmatrix2::intmatrix2(const intmatrix2& m) :
	...
	std::ostream &operator<<(std::ostream &os, const intmatrix2 &m) {
	...
	void intmatrix2::print (const char* matrix_name) const {
	...
	bool intmatrix2::isEqual(intmatrix2& matrix2) {
	...
	intmatrix2 intmatrix2::add(intmatrix2& m) {
		if(this->_rows == m._rows && this->_cols == m._cols) {
			intmatrix2 temp(m._rows,m._cols, 0);
			for (int i = 0; i < _rows; i++) {
				for (int j = 0; j < _cols; j++) {
	...
	intmatrix2 intmatrix2::mult(intmatrix2& m) {
		...
		if(this->_cols == 1 && this->_rows == 1) {
			...
			intmatrix2 temp(this->_rows,this->_cols,0);
